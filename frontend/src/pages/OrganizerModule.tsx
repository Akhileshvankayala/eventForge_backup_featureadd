...
export default function OrganizerModule() {
  ...
  const saveEvent = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const slug = `${title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "event"}-${Date.now().toString(36)}`;
